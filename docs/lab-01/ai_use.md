...
| Evaluate an external reference against approved work | "เรามี Lab1_Starter_Scaffold แล้วเราต้องทำไงต่อ ต้องแก้มั้ยหรือไม่จำเป็น แต่มันจะกระทบทุก issues ที่เราให้เพื่อนตรวจ PR เลยมั้ยนะ" *(the TA just attached a starter scaffold, do we need to fix ours to match, and would it affect the Issues our friend already reviewed?)* | The agent compared every file in the scaffold against what we had already built instead of guessing from the folder names, and the answer turned out to be no. Reworking merged and reviewed code just to match someone else's naming choices would have cost real re-review effort for no actual benefit under the rubric. |
