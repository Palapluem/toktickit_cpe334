...
Playwright runs against a disposable PostgreSQL database whose name ends in `_test`, separate from both the development and API-integration databases. The harness accepts `E2E_DATABASE_URL`, or derives `toktickit_e2e_test` from the credentials in `server/.env.test` when that file points at `toktickit_test`. Global setup applies committed migrations with `prisma migrate deploy` and runs the idempotent seed; it never resets a database automatically. E2E data uses unique summaries so repeated local runs do not depend on test order or delete another run's data.
